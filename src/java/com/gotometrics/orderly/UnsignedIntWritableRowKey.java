...
/** Serialize and deserialize unsigned integers into a variable-length 
 * sortable byte format. 
 *
 * <p>This format ensures that serialized values will sort in their natural 
 * sort order, as determined by (unsigned) integer comparison. NULL
 * values compare less than any non-NULL value. Although we are serializing
 * and deserializing values with a java int, we treat the most significant bit
 * of the int as a data bit, not a sign bit, because this is an unsigned data
 ...
 * contains the number of serialized data bytes, and the 4-7 most significant
 * bits of data.</p>
 * 
 ...
 * encoding lengths contain 4 bits of data.</p>
 *
 ...
 * bits 0-5 are data bits. Otherwise, bits 4-5 specify the length of the 
 * extended length (&gt; 2 byte) encoding. In all cases, bits 0-3 are data bits.
 * </p>
 * 
 ...
 *
 * <h1> Usage </h1>
 * This is the fastest class for storing unsigned integers. It performs no
